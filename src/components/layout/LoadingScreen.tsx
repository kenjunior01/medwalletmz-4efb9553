 ...
 *
 * Design:
 *  - Province-themed gradient background
 *  - ECG heartbeat SVG line animation
 *  - Floating medical particle icons
...
export function LoadingScreen({ message = 'A carregar...' }: { message?: string }) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-emerald-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 overflow-hidden">
      ...
      <div className="absolute top-[20%] left-[15%] w-32 h-32 rounded-full opacity-10 blur-3xl pointer-events-none" style={{ background: 'var(--region-logo-primary, #0D9488)', animation: 'mw-orb-drift 20s ease-in-out infinite' }} />
      <div className="absolute bottom-[25%] right-[10%] w-24 h-24 rounded-full opacity-8 blur-2xl pointer-events-none" style={{ background: 'var(--region-logo-accent, #F59E0B)', animation: 'mw-orb-drift 15s ease-in-out infinite reverse' }} />
      ...
      <div className="flex flex-col items-center gap-5 relative z-10">
        ...
        <div className="relative mw-breathe">
          <MedWalletLogo size={80} variant="icon" animated={true} showText={false} />
          {/* Soft glow behind logo */}
          <div className="absolute inset-0 rounded-full blur-2xl opacity-30 pointer-events-none" style={{ background: 'var(--region-logo-primary, #0D9488)' }} />
        </div>
