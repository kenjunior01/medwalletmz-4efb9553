...
export default function GlobalCommandCenter() {
  ...
  const loadData = async () => {
    ...
    const [cRes, mRes, nRes] = await Promise.all([
      ...
      (supabase as any).from('country_management').select('*, profiles:user_id(full_name, phone)'),
  ...
  const addManager = async (countryId: string) => {
    ...
    try {
      ...
      const { error } = await (supabase as any).from('country_management').insert({
      ...
      await (supabase as any).from('user_roles').upsert({ user_id: userData.user_id, role: 'country_manager' });
  ...
  const removeManager = async (id: string) => {
    const { error } = await (supabase as any).from('country_management').delete().eq('id', id);
