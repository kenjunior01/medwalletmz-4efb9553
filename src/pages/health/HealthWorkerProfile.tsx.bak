...
function DashboardView({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
    >
      {/* Hero: available toggle */}
      <motion.div
        initial={{ opacity: 0, scale: 0.98 }}
        animate={{ opacity: 1, scale: 1 }}
        className={`mb-6 rounded-3xl p-6 sm:p-8 shadow-xl transition-colors ${
          worker.is_available
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/20 backdrop-blur">
              {worker.is_available ? <Power className="h-7 w-7" /> : <PowerOff className="h-7 w-7" />}
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">
                {worker.is_available ? t('healthWorkers.youAreAvailable') : t('healthWorkers.youAreOffline')}
              </h1>
              <p className="text-white/90 text-sm mt-0.5">
                {PROFESSION_LABELS[worker.profession].emoji} {worker.full_name}
              </p>
              <div className="mt-2 flex items-center gap-3 text-xs">
                <span className="inline-flex items-center gap-1 rounded-full bg-white/15 px-2.5 py-0.5 backdrop-blur">
                  <Star className="h-3 w-3 fill-amber-300 text-amber-300" />
                  {(worker.rating ?? 5).toFixed(1)}
                </span>
                <span className="inline-flex items-center gap-1 rounded-full bg-white/15 px-2.5 py-0.5 backdrop-blur">
                  <Calendar className="h-3 w-3" />
                  {worker.total_bookings ?? 0} {t('healthWorkers.bookingsCount')}
                </span>
                {worker.is_available && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-white/15 px-2.5 py-0.5 backdrop-blur">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-300 animate-pulse" />
                    {t('healthWorkers.online')}
                  </span>
                )}
              </div>
            </div>
          ...
          <button
            ...
            className="rounded-xl bg-white px-4 py-2 text-sm font-semibold text-slate-900 shadow hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2"
          >
            {worker.is_available ? t('healthWorkers.goOffline') : t('healthWorkers.goOnline')}
          </button>
        </div>
      </motion.div>
      ...
      <div className="mb-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        <EarningsTile
          label={t('healthWorkers.today')}
          count={earnings?.today_count ?? 0}
          amount={earnings?.today_earnings ?? 0}
          currency={worker.currency ?? 'MZN'}
          color="bg-emerald-50 text-emerald-900"
          icon={<TrendingUp className="h-4 w-4" />}
        />
        <EarningsTile
          label={t('healthWorkers.week')}
          count={earnings?.week_count ?? 0}
          amount={earnings?.week_earnings ?? 0}
          currency={worker.currency ?? 'MZN'}
          color="bg-blue-50 text-blue-900"
          icon={<Calendar className="h-4 w-4" />}
        />
        <EarningsTile
          label={t('healthWorkers.month')}
          count={earnings?.month_count ?? 0}
          amount={earnings?.month_earnings ?? 0}
          currency={worker.currency ?? 'MZN'}
          color="bg-purple-50 text-purple-900"
          icon={<Award className="h-4 w-4" />}
        />
        <EarningsTile
          label={t('healthWorkers.pendingPayout')}
          count={0}
      ...
      <div className="mb-4 inline-flex rounded-xl bg-slate-100 p-1" role="tablist">
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
            activeTab === 'incoming' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-600 hover:text-slate-900'
          }`}
        >
          {t('healthWorkers.tabIncoming')}
          {incoming.length > 0 && (
        ...
        <button
          ...
          className={`rounded-lg px-4 py-2 text-sm font-medium transition ${
            activeTab === 'history' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-600 hover:text-slate-900'
          }`}
        >
          {t('healthWorkers.tabHistory')}
        </button>
      </div>

      {loading ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center" role="status" aria-busy aria-live="polite">
          <RefreshCw className="mx-auto h-6 w-6 animate-spin text-blue-600" />
          <p className="mt-2 text-sm text-slate-600">{t('healthWorkers.loading')}</p>
        </div>
      ) : activeTab === 'incoming' ? (
        incoming.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
            <Calendar className="mx-auto h-8 w-8 text-slate-400" />
            <p className="mt-2 text-sm text-slate-600">{t('healthWorkers.noIncomingBookings')}</p>
          </div>
        ) : (
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center">
          <Calendar className="mx-auto h-8 w-8 text-slate-400" />
          <p className="mt-2 text-sm text-slate-600">{t('healthWorkers.noHistory')}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {history.map((b, idx) => (
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="rounded-xl bg-emerald-50 p-3 text-xs text-emerald-800 flex items-center gap-2">
          <ShieldCheck className="h-4 w-4 shrink-0" />
          {t('healthWorkers.trustVerifiedWorker')}
        </div>
        <div className="rounded-xl bg-blue-50 p-3 text-xs text-blue-800 flex items-center gap-2">
          <Wallet className="h-4 w-4 shrink-0" />
          {t('healthWorkers.trustDailyPayout')}
        </div>
        <div className="rounded-xl bg-amber-50 p-3 text-xs text-amber-800 flex items-center gap-2">
          <Award className="h-4 w-4 shrink-0" />
          {t('healthWorkers.trustFiveStarBonus')}
        </div>
      </div>
    </motion.div>
  );
        ...
        {booking.status === 'completed' && booking.rating && (
          <div className="flex items-center gap-1 text-xs text-amber-700">
            ...
            {booking.rating_comment && (
              <span className="ml-2 italic text-slate-600 truncate">"{booking.rating_comment}"</span>
            )}
          </div>
        )}
      </div>
